...
sankey_gen <- function(html = FALSE){
  ...
  # total_val <- sum(links$value)
  # print(total_val)
  # print(links$value)
  # links <- links %>%
  #   mutate(probability = value/total_val)
  # print(links$probability)
  ...
  node_data <- data.frame(node = c(0:(total_nodes_no - 1)), name = total_nodes, stringsAsFactors = FALSE) 
  ...
  # # Summarize data to calculate values and probabilities for links
  # links <- links %>%
  #   group_by(NODE_S_ENCODED, NODE_E_ENCODED, PATHNO_ENCODED) %>%
  #   summarise(value = sum(value), .groups = 'drop') %>%
  #   mutate(probability = value / sum(value), value = value)
  # 
  # # Calculate the total value within each PATHNO_ENCODED
  # total_counts_links <- links %>%
  #   group_by(PATHNO_ENCODED) %>%
  #   summarise(total_count = sum(value), .groups = 'drop')
  # 
  # # Join the total_counts back to the original data to normalize the probability
  # links <- links %>%
  #   left_join(total_counts_links, by = "PATHNO_ENCODED") %>%
  #   mutate(probability = value / total_count, value = value) %>%
  #   select(-total_count)
  # 
  # # Remove duplicates if needed
  # links <- links %>% filter(probability != 0)
  # 
  # # Convert columns to numeric if necessary
  # links$probability <- as.numeric(links$probability)
  # links$value <- as.numeric(links$value)
  # links$PATHNO_ENCODED <- as.integer(links$PATHNO_ENCODED)
  # 
  # # Order the links by PATHNO_ENCODED, source, and target
  # links <- links[order(links$PATHNO_ENCODED, links$NODE_S_ENCODED, links$NODE_E_ENCODED), ]
  # 
  # # Summarize total probability and total counts within each PATHNO_ENCODED
  # links %>%
  #   group_by(PATHNO_ENCODED) %>%
  #   summarise(total_probability = sum(probability, na.rm = TRUE), total_counts = sum(value, na.rm = TRUE))
  ...
  # For each path2, the data is grouped by the unique_path_no and the number of each path2 is given as well as the path2 number
  for (i in 1:no_of_paths2){
    sankey_data_p <- sankey_data_subset %>%
      filter(PATHNO == unique_path_no[i]) %>%
      group_by(source, target, probability, value) %>%
    ...
    if (i == 1){
      sankey_data_p <- sankey_data_p %>%
        merge(node_data_ord[1:total_num_list[1],], by.x = "source", by.y = "name", all = TRUE) %>%
        merge(node_data_ord[(total_num_list[1] + 1):total_num_list[2],], by.x = "target", by.y = "name", all = TRUE)
    ...
    # it to the path_links variable.
      ...
      sankey_data_p <- sankey_data_p %>%
        merge(node_data_ord[(total_num_list[i - 1] + 1):total_num_list[i],], by.x = "source", by.y = "name", all = TRUE) %>%
        merge(node_data_ord[(total_num_list[i] + 1):total_num_list[i + 1],], by.x = "target", by.y = "name", all = TRUE)
      
      path_links <- rbind(path_links, sankey_data_p)
    }
  }
  
  # Orders the path_links variable by the path2 number and its probability (number of those path2)
  path_links <- path_links[order(path_links$PATHNO, path_links$probability),]
  
  # Create the colour scale for nodes
  colour_scheme <- 'd3.scaleOrdinal()
                .domain(["IDFS", "MR", "NMR", "Death"])
                .range(["green", "orange", "yellow", "red"]);'
  
  path_links <- path_links %>%
    mutate(TargetName = target)
  ...
  # svg_width <- 800 + margin$left + margin$right
  
  # print(head(nodedata_ord))
  #print(head(node_data_ord2))
  # print(head(node_probabilities))
  # print(head(node_data_ord))
  # print(nodedata_ord$timepoint)
  # print(class(nodedata_ord$timepoint))
  # print(nodedata_ord$group)
  # print(class(nodedata_ord$group))
  # print(head(path_links))
  # print(head(links))
  # print(svg_height)
  # print(svg_width)
  # print(class(svg_height))
  # print(class(svg_width))
  # print(head(links))
  #print(head(data_p))
  #print(head(path_links))
  # print(head(path_links))
  
  # links <- full_join(path_links, links,
  #                    by=c('target'='NODE_E_ENCODED',
  #                         'source'='NODE_S_ENCODED',
  #                         'value',
  #                         'PATHNO'= 'PATHNO_ENCODED',
  #                         'node.x',
  #                         'node_ord.x',
  #                         'node.y',
  #                         'node_ord.y'))
  
  # nodedata_ord <- full_join(node_data_ord2, nodedata_ord,
  #                           by=c('node',
  #                                'name',
  #                                'node_ord',
  #                                'value' = 'size'))
